...
- (void)viewDidLoad {
	...
	UIImageView *t_imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, kScreenHeitht)];
	[t_imageView setImage:[UIImage imageNamed:@"城市管理背景.jpg"]];
	[self.view addSubview:t_imageView];
//	[t_imageView release];
	
	UIImageView *t_editBarBg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0+y, self.view.width, 44)];
	t_editBarBg.image = [UIImage imageNamed:@"top.jpg"];
	[self.view addSubview:t_editBarBg];
//	[t_editBarBg release];
	
	UILabel *t_title = [[UILabel alloc] initWithFrame:CGRectMake(0, 0+y, self.view.width, 44)];
	...
	[t_title setFont:[UIFont boldSystemFontOfSize:18]];
	switch (shareType) {
		case Sina:
			t_title.text = @"新浪微博";
			break;
...
- (void)request:(SinaWeiboRequest *)request didFailWithError:(NSError *)error
{
	...
	if ([t_str intValue] == 20019) {
		[ShareFun alertNotice:@"知天气" withMSG:@"新浪微博发送失败，内容重复" cancleButtonTitle:@"确定" otherButtonTitle:nil];
	}
	else if ([t_str intValue] == 20016) {
		[ShareFun alertNotice:@"知天气" withMSG:@"发送过于频繁，请稍后再试" cancleButtonTitle:@"确定" otherButtonTitle:nil];
	}
	else {
		[ShareFun alertNotice:@"知天气" withMSG:@"新浪微博发送失败，请重新发送" cancleButtonTitle:@"确定" otherButtonTitle:nil];
	}
...
- (void)renren:(Renren *)renren loginFailWithError:(ROError*)error
{
	...
	[ShareFun alertNotice:@"知天气" withMSG:@"人人网绑定失败，请重新尝试" cancleButtonTitle:@"确定" otherButtonTitle:nil];
}
...
- (void)dealWithError:(ROError *)error
{
    ...
    [ShareFun alertNotice:@"知天气" withMSG:errorAlertString cancleButtonTitle:@"确定" otherButtonTitle:nil];
}
...
- (void)onFailureLogin:(NSError *)error
{
	...
	[ShareFun alertNotice:@"知天气" withMSG:@"腾讯微博绑定失败，请重新尝试" cancleButtonTitle:@"确定" otherButtonTitle:nil];
}
...
- (void)failureCallBack:(NSError *)error{
    NSLog(@"error: %@", error);
	[ShareFun alertNotice:@"知天气" withMSG:@"腾讯微博发送失败，请重新尝试" cancleButtonTitle:@"确定" otherButtonTitle:nil];
	[self performSelector:@selector(statusBarHide)];
}
