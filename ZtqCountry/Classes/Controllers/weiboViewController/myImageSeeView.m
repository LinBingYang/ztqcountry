...
//		[_scrollView release];
		...
		//_scrollView.contentSize = CGSizeMake(320, [self heightOfContent]);
		...
		[_scrollView addSubview:m_imageView];
//		[m_imageView release];
...
- (id)initWithImageName:(NSString *)imageName
{
    if ((self = [self init]))
    {
		NSString *shareImagePath = [NSTemporaryDirectory() stringByAppendingPathComponent:imageName];
		[m_imageView setImage:[UIImage imageWithContentsOfFile:shareImagePath]];
    }
    return self;
}
...
- (void)show
{
	...
	_scrollView.frame = CGRectMake(kBorderWidth+1, kBorderWidth+1, innerWidth,
								   self.frame.size.height - (1 + kBorderWidth*2));
	m_imageView.frame = CGRectMake(0, 0, innerWidth,
                               self.frame.size.height - (1 + kBorderWidth*2));
