...
#define INTERVALE 5
@interface ViedoADSrcollView()<UIScrollViewDelegate>
{
...
@implementation ViedoADSrcollView
...
- (instancetype)initWithNameArr:(NSMutableArray *)imageArr titleArr:(NSMutableArray *)titleArr height:(float)heightValue offsetY:(CGFloat)offsetY  offsetx:(CGFloat)offsetx{
    ...
    self = [super initWithFrame:CGRectMake(offsetx, offsetY, WIDTH, heightValue)];
    //    self.x=offsetx;
    if (self) {
...
//            NSURL *url=[NSURL URLWithString:imageURL];
            ...
            UIImageView *closeimg=[[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth-30, 5, 25, 25)];
            closeimg.image=[UIImage imageNamed:@"banner清除常态"];
            closeimg.userInteractionEnabled=YES;
            [egoimg addSubview:closeimg];
            
            UIButton *closebtn = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth-50 , 0, 50,50)];
//            [closebtn setBackgroundImage:[UIImage imageNamed:@"banner清除常态"] forState:UIControlStateNormal];
//            [closebtn setBackgroundImage:[UIImage imageNamed:@"banner清除二态"] forState:UIControlStateHighlighted];
            [closebtn addTarget:self action:@selector(closeview) forControlEvents:UIControlEventTouchUpInside];
            [egoimg addSubview:closebtn];
...
{
    CGRect rect =  CGRectMake(250, 140, 70, 30);
...
-(void)click:(int)vid{
    //调用委托实现方法
    [self.vDelegate YSBanerbuttonClick:vid];
}
-(void)clickbtn:(UIButton *)sender{
    NSInteger tag=sender.tag;
    [self.vDelegate YSBanerbuttonClick:tag];
}
-(void)closeview{
    [setting sharedSetting].fs=@"关闭";
