...
static MFSideMenuContainerViewController *shareVC = nil;
...
+ (MFSideMenuContainerViewController *)containerWithCenterViewController:(id)centerViewController
    ...
    @synchronized(self)
    {
        if(shareVC == nil)
        {
            shareVC = [MFSideMenuContainerViewController new];
            shareVC.leftMenuViewController = leftMenuViewController;
            shareVC.centerViewController = centerViewController;
            
            shareVC.rightMenuViewController = rightMenuViewController;
        }
    }
    return shareVC;
}
...
- (void)setDefaultSettings {
    ...
    self.menuWidth = kScreenWidth/6*5;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //    [self closeSideMenuCompletion:nil];
}
...
- (void)setCenterViewController:(UIViewController *)centerViewController {
    ...
    //    CGRect center = centerViewController.view.frame;
    
    [self.view addSubview:[_centerViewController view]];
    [_centerViewController didMoveToParentViewController:self];
    
    [self setCenterMenuFrame];
}

- (void)setCenterMenuFrame
{
    CGRect rectV = self.view.frame;
    rectV.size.height = rectV.size.height + rectV.origin.y;
    rectV.origin.y = 0;
    self.view.frame = rectV;
    
    [self.centerViewController view].frame = rectV;
    [self.centerViewController view].autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleRightMargin;
}
...
- (void)addGestureRecognizers {
    ...
    //    [[self.centerViewController view] addGestureRecognizer:leftSwipeRecognizer];
    //
    ...
    //    [[self.centerViewController view] addGestureRecognizer:rightSwipeRecognizer];
...
-(void)togleCenterViewController
{
...
- (void)setMenuState:(MFSideMenuState)menuState completion:(void (^)(void))completion {
    void (^innerCompletion)() = ^ {
        ...
        NSDictionary * info = [[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithInt:menuState],@"state", nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"MFSlideState" object:nil userInfo:info];
...
- (void) setRightSideMenuFrameToClosedPosition {
    ...
    [self.rightMenuViewController view].autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
-(void)centerViewControllerLeftSwiped:(id)sender{
    if(self.menuState == MFSideMenuStateLeftMenuOpen &&self.menuState != MFSideMenuStateClosed)
    {
        [self setMenuState:MFSideMenuStateClosed];
    }
}
-(void)centerViewControllerRighttSwiped:(id)sender{
    if(self.menuState == MFSideMenuStateRightMenuOpen &&self.menuState != MFSideMenuStateClosed)
    {
        [self setMenuState:MFSideMenuStateClosed];
    }
}
...
+ (MFSideMenuContainerViewController *)share
{
    return shareVC;
}
