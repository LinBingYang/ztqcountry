...
//    [gz_todaywt_inde setObject:[setting sharedSetting].currentCityID forKey:@"county_id"];
    ...
    [[NetWorkCenter share] postHttpWithUrl:urlStr withParam:param withFlag:1 withSuccess:^(NSDictionary *returnData) {
        ...
        if (b!=nil) {
            ...
            UIImage *t_shareImage1=[self makeImageMore:t_shareImage];
            self.shareimg=t_shareImage1;
            NSString *shareImagePath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"weiboShare.png"];
            if ([UIImagePNGRepresentation(t_shareImage1) writeToFile:shareImagePath atomically:YES])
                NSLog(@">>write ok");
...
-(void)selectTable:(NSString *)t_city isProvince:(BOOL)isProvince{
    ...
    if (isProvince) {
        //跳转到省份的界面
        NSLog(@"%@", t_city);
        AirRankProvinceVC *rankPrienceVC = [[AirRankProvinceVC alloc]init];
        rankPrienceVC.isProvince = YES;
        rankPrienceVC.selectProvinceName = t_city;
        rankPrienceVC.rankType = _rankType;
        [self.navigationController pushViewController:rankPrienceVC animated:YES];
    }else {
...
//        rank_lable.shadowColor = [UIColor grayColor];
        rank_lable.text=[NSString stringWithFormat:@"%d",rlm.rank];
