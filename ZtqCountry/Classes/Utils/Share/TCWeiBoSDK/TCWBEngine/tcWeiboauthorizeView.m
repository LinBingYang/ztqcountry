//
//  tcWeiboauthorizeView.m
//  ztq_heilj
//
//  Created by lihj on 12-11-1.
//  Copyright 2012 __MyCompanyName__. All rights reserved.
//

#import "tcWeiboauthorizeView.h"
...
@implementation tcWeiboauthorizeView
...
- (id)initWithURLString:(NSString *)string
                delegate:(id<tcWeiboauthorizeViewDelegate>)_delegate
{
    if ((self = [self init]))
    {
...
- (void)sizeToFitOrientation:(BOOL)transform
{
    ...
    CGFloat scaleFactor = tcWeiboIsDeviceIPad() ? 0.6f : 1.0f;
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (range.location != NSNotFound){
		...
		if (code){
			...
			if (err == nil) {
				err = [NSError errorWithDomain:TCWBSDKErrorDomain 
			...
			if([delegate respondsToSelector:@selector(authorize:didFailuredWithError:)]){
				[self hide];
				[delegate authorize:self didFailuredWithError:err];
			}
		}
		return NO;
    }
