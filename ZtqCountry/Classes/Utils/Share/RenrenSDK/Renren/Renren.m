...
//    RORequestParam * param = [[[RORequestParam alloc] init] autorelease];
    RORequestParam * param = [[RORequestParam alloc] init];
...
- (void)authDialog:(id)dialog withOperateType:(RODialogOperateType )operateType{
    ...
    } else if ([dialog isKindOfClass:[RenRenAuthorizeView class]]) {
        RenRenAuthorizeView *viewController = (RenRenAuthorizeView *)dialog;
        response = viewController.response;
    }
                                   ...
                                   andDelegate:(id<RenrenDelegate>)delegate
{
    ...
    if (![self isSessionValid]) {
		...
		RenRenAuthorizeView *authorizeView = [[RenRenAuthorizeView alloc] initWithAuthParams];
		authorizeView.delegate = self;
		authorizeView.params = parameters;
		authorizeView.serverURL = kAuthBaseURL;
		[authorizeView show];
...
- (void)dialogInNavigation:(NSString *)action andParams:(NSMutableDictionary *)params andDelegate:(id <RenrenDelegate>)delegate
{
    ...
    RenRenAuthorizeView *authorizeView = [[RenRenAuthorizeView alloc] initWithAuthParams];
    authorizeView.delegate = self;
    authorizeView.params = params;
    authorizeView.serverURL = dialogURL;
    [authorizeView show];
//    [authorizeView release];
