...
@synthesize targetForExecution = targetForExecution;
@synthesize objectForExecution = objectForExecution;
...
- (void)updateIndicators {
    ...
    if ([self.labelText isEqualToString:@"缓存清理完成"]) {
...
+ (MBProgressHUD *)showViewCenterHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithCenterView:view];
    hud.labelText = @"加载中...";
    [view addSubview:hud];
    [hud show:animated];
    //	return [hud autorelease];
...
- (id)initWithView:(UIView *)view {
	...
	id me = [self initWithFrame:CGRectMake(0, 100, kScreenWidth, kScreenHeitht-100)];
...
- (id)initWithCenterView:(UIView *)view {
    // Let's check if the view is nil (this is a common error when using the windw initializer above)
    if (!view) {
        [NSException raise:@"MBProgressHUDViewIsNillException"
                    format:@"The view used in the MBProgressHUD initializer is nil."];
    }
    id me = [self initWithFrame:CGRectMake(0, -100, kScreenWidth, kScreenHeitht)];
    // We need to take care of rotation ourselfs if we're adding the HUD to a window
    if ([view isKindOfClass:[UIWindow class]]) {
        [self setTransformForCurrentOrientation:NO];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deviceOrientationDidChange:)
