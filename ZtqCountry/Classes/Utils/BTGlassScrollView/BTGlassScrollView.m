...
- (void)createForegroundView
{
    ...
    [_foregroundScrollView setDirectionalLockEnabled:YES];
...
- (void)createBottomShadow
{
    ...
    _botShadowLayer = [self createTopMaskWithSize:CGSizeMake(self.frame.size.width,_viewDistanceFromBottom) startFadeAt:0 endAt:_viewDistanceFromBottom topColor:[UIColor colorWithWhite:0 alpha:0] botColor:[UIColor colorWithWhite:0 alpha:0]];
    [_botShadowLayer setFrame:CGRectOffset(_botShadowLayer.bounds, 0, self.frame.size.height - _viewDistanceFromBottom)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    CGFloat ratio = (scrollView.contentOffset.y + _foregroundScrollView.contentInset.top)/(_foregroundScrollView.frame.size.height - _foregroundScrollView.contentInset.top - 120);
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    CGPoint srcpoint=scrollView.contentOffset;
    ...
    if (srcpoint.y>=500) {
        ...
        _foregroundScrollView.bounces=YES;
        if (scrollView.contentOffset.y<-124) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"refreshview" object:nil];
        }
    }
    CGFloat ratio = (point.y + _foregroundScrollView.contentInset.top)/(_foregroundScrollView.frame.size.height - _foregroundScrollView.contentInset.top - 120);
